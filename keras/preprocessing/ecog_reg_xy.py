    ...
    for f, file_ind in enumerate(index_array):
        ...
        try:
            ...
            while ydata_end[0] < 0:
                t -= 1
                ydata_end = ydata[t]
            ydata_end[0] = ydata_end[0] * (64 / 640.0)
            ydata_end[1] = ydata_end[1] * (64 / 480.0)
            ...
            batch_y[:, f] = (angle_between(ydata_start, ydata_end)/360.0, min(np.sum(np.abs(ydata_end-ydata_start))/4.0,1.0))
        except:
            pdb.set_trace()
...
class DirectoryIterator(Iterator):
    ...
    def __init__(self, directory, EcogDataGenerator,
        ...
        subpath = os.path.join(directory, 'X')
